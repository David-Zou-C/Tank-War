...
class RandomCreate:
    ...
    def Tank(self, Num, speed, NoCollideGroup):
        ...
        #     tank = Tank(all_image.TankImage, position, speed, f'random-only1', 1,
        ...
        for i in range(Num):
            position = random.randint(0, self.mapWidth - imageWidth), \
                       random.randint(0, self.mapHeight - imageHeight)
            tank = Tank(all_image.TankImage, position, speed, f'random-{i}', i,
                        direction=random.choice(['up', 'down', 'left', 'right']))
            while self.Collide(tank, Tank_Group) or self.Collide(tank, NoCollideGroup):
                tank.rect.left, tank.rect.top = random.randint(0, self.mapWidth - imageWidth), \
                                                random.randint(0, self.mapHeight - imageHeight)
...
class Tank(DirtySprite):
    ...
    def __init__(self, image, position, speed, tank_name, Tank_ID, direction='up', layer=all_layer.TankLayer):
    ...
    def get_state(self):
        res_dic = {}
        res_dic['event type'] = 'tank state'
        res_dic['Tank ID'] = self.tank_ID
        res_dic['life'] = self.life
        res_dic['total life'] = self.totalLife
        res_dic['skill'] = self.skill
        res_dic['total skill'] = self.totalSkill
        res_dic['position'] = self.rect.left, self.rect.top
        res_dic['direction'] = self.direction
        return res_dic

    def change_state(self, info):
        if info['event type'] == 'tank state' and info['Tank ID'] == self.tank_ID:
            self.life = info['life']
            self.skill = info['skill']
            self.rect.left, self.rect.top = info['position']
